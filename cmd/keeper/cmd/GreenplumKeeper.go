...
import (
	...
	gp "github.com/sorintlab/stolon/internal/greenplum"
...
const (
	maxGreenplumTimelinesHistory = 2
...
var gpcfg greenplumConfig

func registerGreenplumCmd(root *cobra.Command) {
	var gpKeeperCmd = &cobra.Command{
		Use: "greenplum",
		Run: greenplumKeeper,
		//Version: cmd.Version,
	}

	cmd.AddCommonFlags(gpKeeperCmd, &cfg.CommonConfig)

	gpKeeperCmd.PersistentFlags().StringVar(&cfg.uid, "id", "", "keeper uid (must be unique in the cluster and can contain only lower-case letters, numbers and the underscore character). If not provided a random uid will be generated.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.uid, "uid", "", "keeper uid (must be unique in the cluster and can contain only lower-case letters, numbers and the underscore character). If not provided a random uid will be generated.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.dataDir, "data-dir", "", "data directory")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgListenAddress, "gp-listen-address", "", "postgresql instance listening address, local address used for the postgres instance. For all network interface, you can set the value to '*'.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgAdvertiseAddress, "gp-advertise-address", "", "postgresql instance address from outside. Use it to expose ip different than local ip with a NAT networking config")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgPort, "gp-port", "5432", "postgresql instance listening port")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgAdvertisePort, "gp-advertise-port", "", "postgresql instance port from outside. Use it to expose port different than local port with a PAT networking config")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgBinPath, "gp-bin-path", "", "absolute path to postgresql binaries. If empty they will be searched in the current PATH")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplAuthMethod, "gp-repl-auth-method", "md5", "postgres replication user auth method. Default is md5.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplUsername, "gp-repl-username", "", "postgres replication user name. Required. It'll be created on db initialization. Must be the same for all keepers.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplPassword, "gp-repl-password", "", "postgres replication user password. Only one of --gp-repl-password or --gp-repl-passwordfile must be provided. Must be the same for all keepers.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplPasswordFile, "gp-repl-passwordfile", "", "postgres replication user password file. Only one of --gp-repl-password or --gp-repl-passwordfile must be provided. Must be the same for all keepers.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUAuthMethod, "gp-su-auth-method", "md5", "postgres superuser auth method. Default is md5.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUUsername, "gp-su-username", "", "postgres superuser user name. Used for keeper managed instance access and pg_rewind based synchronization. It'll be created on db initialization. Defaults to the name of the effective user running stolon-keeper. Must be the same for all keepers.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUPassword, "gp-su-password", "", "postgres superuser password. Only one of --gp-su-password or --gp-su-passwordfile must be provided. Must be the same for all keepers.")
	gpKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUPasswordFile, "gp-su-passwordfile", "", "postgres superuser password file. Only one of --gp-su-password or --gp-su-passwordfile must be provided. Must be the same for all keepers)")
	gpKeeperCmd.PersistentFlags().BoolVar(&cfg.debug, "debug", false, "enable debug logging")

	gpKeeperCmd.PersistentFlags().BoolVar(&cfg.canBeMaster, "can-be-master", true, "prevent keeper from being elected as master")
	gpKeeperCmd.PersistentFlags().BoolVar(&cfg.canBeSynchronousReplica, "can-be-synchronous-replica", true, "prevent keeper from being chosen as synchronous replica")
	gpKeeperCmd.PersistentFlags().BoolVar(&cfg.disableDataDirLocking, "disable-data-dir-locking", false, "disable locking on data dir. Warning! It'll cause data corruptions if two keepers are concurrently running with the same data dir.")

	if err := gpKeeperCmd.PersistentFlags().MarkDeprecated("id", "please use --uid"); err != nil {
		log.Fatal(err)
	}
	if err := gpKeeperCmd.PersistentFlags().MarkDeprecated("debug", "use --log-level=debug instead"); err != nil {
		log.Fatal(err)
	}

	root.AddCommand(gpKeeperCmd)
}

// walLevel returns the wal_level value to use.
// if there's an user provided wal_level gp parameters and if its value is
// "logical" then returns it, otherwise returns the default ("hot_standby" for
// gp < 9.6 or "replica" for gp >= 9.6).
func (p *GreenplumKeeper) walLevel(db *cluster.DB) string {
...
func (p *GreenplumKeeper) walKeepSegments(db *cluster.DB) int {
...
func (p *GreenplumKeeper) walKeepSize(db *cluster.DB) string {
...
func (p *GreenplumKeeper) mandatoryPGParameters(db *cluster.DB) common.Parameters {
...
func (p *GreenplumKeeper) getSUConnParams(db, followedDB *cluster.DB) gp.ConnParams {
	cp := gp.ConnParams{
		"user":             p.pgSUUsername,
...
func (p *GreenplumKeeper) getReplConnParams(db, followedDB *cluster.DB) gp.ConnParams {
	cp := gp.ConnParams{
...
func (p *GreenplumKeeper) getLocalConnParams() gp.ConnParams {
	cp := gp.ConnParams{
		"user":   p.pgSUUsername,
...
func (p *GreenplumKeeper) getLocalReplConnParams() gp.ConnParams {
	cp := gp.ConnParams{
		"user":     p.pgReplUsername,
...
func (p *GreenplumKeeper) createPGParameters(db *cluster.DB) common.Parameters {
	...
	// Copy user defined gp parameters
	for k, v := range db.Spec.PGParameters {
		parameters[k] = v
	}

	// Add/Replace mandatory DBMSParameters
...
func (p *GreenplumKeeper) createRecoveryOptions(recoveryMode dbmgr.RecoveryMode, standbySettings *cluster.StandbySettings, archiveRecoverySettings *cluster.ArchiveRecoverySettings, recoveryTargetSettings *cluster.RecoveryTargetSettings) *dbmgr.RecoveryOptions {
	parameters := common.Parameters{}
	...
	return &dbmgr.RecoveryOptions{
		RecoveryMode:       recoveryMode,
		RecoveryParameters: parameters,
	}
}

type GreenplumKeeper struct {
	cfg *greenplumConfig
...
var _ KeeperIF = (*GreenplumKeeper)(nil)

func NewGreenplumKeeper(cfg *greenplumConfig, end chan error) (KeeperIF, error) {
...
func greenplumKeeper(c *cobra.Command, args []string) {
	var (
		err           error
	...
	p, err := NewGreenplumKeeper(&gpcfg, end)
...
func (p *GreenplumKeeper) dbLocalStateCopy() *DBLocalState {
	p.localStateMutex.Lock()
	defer p.localStateMutex.Unlock()
	return p.dbLocalState.DeepCopy()
}

func (p *GreenplumKeeper) usePgrewind(db *cluster.DB) bool {
	return p.pgSUUsername != "" && p.pgSUPassword != "" && db.Spec.UsePgrewind
}

func (p *GreenplumKeeper) updateKeeperInfo() error {
	p.localStateMutex.Lock()
	...
	keeperInfo := &cluster.KeeperInfo{
		...
		BootUUID:   p.bootUUID,
		DBMSBinaryVersion: cluster.DBMSBinaryVersion{
			Maj: maj,
			Min: min,
		},
		DBMSState: p.getLastPGState(),
...
func (p *GreenplumKeeper) updatePGState(pctx context.Context) {
	p.pgStateMutex.Lock()
	defer p.pgStateMutex.Unlock()
	pgState, err := p.GetPGState(pctx)
	if err != nil {
		log.Errorw("failed to get gp state", zap.Error(err))
	}
	p.lastPGState = pgState
}

func (p *GreenplumKeeper) GetInSyncStandbys() ([]string, error) {
...
func (p *GreenplumKeeper) GetPGState(pctx context.Context) (*cluster.DBMSState, error) {
	...
	if initialized {
		...
		if err != nil {
			log.Errorw("cannot get configured gp parameters", zap.Error(err))
			return pgState, nil
		}
		log.Debugw("got configured gp parameters", "pgParameters", pgParameters)
		...
		log.Debugw("filtered out managed gp parameters", "filteredPGParameters", filteredPGParameters)
		...
		if err != nil {
			log.Errorw("error getting gp state", zap.Error(err))
			return pgState, nil
		}
		...
		ctlsh, err := getTimeLinesHistory(pgState, p.pgm, maxGreenplumTimelinesHistory)
		if err != nil {
...
func (p *GreenplumKeeper) getLastPGState() *cluster.DBMSState {
	p.pgStateMutex.Lock()
...
func (p *GreenplumKeeper) Start(ctx context.Context) {
	...
	pgm := gp.NewManager(p.pgBinPath, p.dataDir, p.getLocalConnParams(), p.getLocalReplConnParams(), p.pgSUAuthMethod, p.pgSUUsername, p.pgSUPassword, p.pgReplAuthMethod, p.pgReplUsername, p.pgReplPassword, p.requestTimeout)
	p.pgm = pgm
	...
	for {
		...
		case <-ctx.Done():
			...
			if err = p.pgm.StopIfStarted(true); err != nil {
				log.Errorw("failed to stop gp instance", zap.Error(err))
			}
			p.end <- nil
			return
...
func (p *GreenplumKeeper) resync(db, masterDB, followedDB *cluster.DB, tryPgrewind bool) error {
	pgm := p.pgm
	...
	if tryPgrewind && p.usePgrewind(db) {
		...
		if err := pgm.SyncFromFollowedPGRewind(connParams, p.pgSUPassword); err != nil {
			...
			pgm.SetRecoveryOptions(p.createRecoveryOptions(dbmgr.RecoveryModeStandby, standbySettings, nil, nil))
			return nil
		}
	}
...
// TODO(sgotti) unify this with the sentinel one. They have the same logic but one uses *cluster.DBMSState while the other *cluster.DB
func (p *GreenplumKeeper) isDifferentTimelineBranch(followedDB *cluster.DB, pgState *cluster.DBMSState) bool {
...
func (p *GreenplumKeeper) updateReplSlots(curReplSlots []string, uid string, followersUIDs, additionalReplSlots []string) error {
...
func (p *GreenplumKeeper) refreshReplicationSlots(cd *cluster.ClusterData, db *cluster.DB) error {
...
func (p *GreenplumKeeper) postgresKeeperSM(pctx context.Context) {
	e := p.e
	pgm := p.pgm
	...
	if p.dbLocalState.UID != db.UID {
		...
		case cluster.DBInitModePITR:
			...
			recoveryMode := dbmgr.RecoveryModeRecovery
			...
			if db.Spec.FollowConfig != nil && db.Spec.FollowConfig.Type == cluster.FollowTypeExternal {
				recoveryMode = dbmgr.RecoveryModeStandby
			...
			if recoveryMode == dbmgr.RecoveryModeRecovery {
	...
	if !initialized {
		...
		if err != nil {
			log.Errorw("error retrieving current gp role", zap.Error(err))
			return
		}
	}
	...
	case common.RoleStandby:
		...
		case common.RoleStandby:
			...
			case cluster.FollowTypeInternal:
				...
				newRecoveryOptions := p.createRecoveryOptions(dbmgr.RecoveryModeStandby, standbySettings, nil, nil)
			...
			case cluster.FollowTypeExternal:
				curRecoveryOptions := pgm.CurRecoveryOptions()
				newRecoveryOptions := p.createRecoveryOptions(dbmgr.RecoveryModeStandby, db.Spec.FollowConfig.StandbySettings, db.Spec.FollowConfig.ArchiveRecoverySettings, nil)
...
func (p *GreenplumKeeper) keeperLocalStateFilePath() string {
	return filepath.Join(p.cfg.dataDir, "keeperstate")
}

func (p *GreenplumKeeper) loadKeeperLocalState() error {
...
func (p *GreenplumKeeper) saveKeeperLocalState() error {
...
func (p *GreenplumKeeper) dbLocalStateFilePath() string {
	return filepath.Join(p.cfg.dataDir, "dbstate")
}

func (p *GreenplumKeeper) loadDBLocalState() error {
...
func (p *GreenplumKeeper) saveDBLocalState(dbls *DBLocalState) error {
...
func (p *GreenplumKeeper) generateHBA(cd *cluster.ClusterData, db *cluster.DB, onlyInternal bool) []string {
