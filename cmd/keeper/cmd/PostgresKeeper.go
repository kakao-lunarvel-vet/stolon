...
func registerPostgresCmd(root *cobra.Command) {
	var pgKeeperCmd = &cobra.Command{
		Use: "postgres",
		Run: postgresKeeper,
		//Version: cmd.Version,
	}

	cmd.AddCommonFlags(pgKeeperCmd, &cfg.CommonConfig)

	pgKeeperCmd.PersistentFlags().StringVar(&cfg.uid, "id", "", "keeper uid (must be unique in the cluster and can contain only lower-case letters, numbers and the underscore character). If not provided a random uid will be generated.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.uid, "uid", "", "keeper uid (must be unique in the cluster and can contain only lower-case letters, numbers and the underscore character). If not provided a random uid will be generated.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.dataDir, "data-dir", "", "data directory")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgListenAddress, "pg-listen-address", "", "postgresql instance listening address, local address used for the postgres instance. For all network interface, you can set the value to '*'.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgAdvertiseAddress, "pg-advertise-address", "", "postgresql instance address from outside. Use it to expose ip different than local ip with a NAT networking config")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgPort, "pg-port", "5432", "postgresql instance listening port")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgAdvertisePort, "pg-advertise-port", "", "postgresql instance port from outside. Use it to expose port different than local port with a PAT networking config")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgBinPath, "pg-bin-path", "", "absolute path to postgresql binaries. If empty they will be searched in the current PATH")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplAuthMethod, "pg-repl-auth-method", "md5", "postgres replication user auth method. Default is md5.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplUsername, "pg-repl-username", "", "postgres replication user name. Required. It'll be created on db initialization. Must be the same for all keepers.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplPassword, "pg-repl-password", "", "postgres replication user password. Only one of --pg-repl-password or --pg-repl-passwordfile must be provided. Must be the same for all keepers.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgReplPasswordFile, "pg-repl-passwordfile", "", "postgres replication user password file. Only one of --pg-repl-password or --pg-repl-passwordfile must be provided. Must be the same for all keepers.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUAuthMethod, "pg-su-auth-method", "md5", "postgres superuser auth method. Default is md5.")
	...
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUPassword, "pg-su-password", "", "postgres superuser password. Only one of --pg-su-password or --pg-su-passwordfile must be provided. Must be the same for all keepers.")
	pgKeeperCmd.PersistentFlags().StringVar(&cfg.pgSUPasswordFile, "pg-su-passwordfile", "", "postgres superuser password file. Only one of --pg-su-password or --pg-su-passwordfile must be provided. Must be the same for all keepers)")
	pgKeeperCmd.PersistentFlags().BoolVar(&cfg.debug, "debug", false, "enable debug logging")

	pgKeeperCmd.PersistentFlags().BoolVar(&cfg.canBeMaster, "can-be-master", true, "prevent keeper from being elected as master")
	pgKeeperCmd.PersistentFlags().BoolVar(&cfg.canBeSynchronousReplica, "can-be-synchronous-replica", true, "prevent keeper from being chosen as synchronous replica")
	pgKeeperCmd.PersistentFlags().BoolVar(&cfg.disableDataDirLocking, "disable-data-dir-locking", false, "disable locking on data dir. Warning! It'll cause data corruptions if two keepers are concurrently running with the same data dir.")

	if err := pgKeeperCmd.PersistentFlags().MarkDeprecated("id", "please use --uid"); err != nil {
		log.Fatal(err)
	}
	if err := pgKeeperCmd.PersistentFlags().MarkDeprecated("debug", "use --log-level=debug instead"); err != nil {
		log.Fatal(err)
	}

	root.AddCommand(pgKeeperCmd)
}
...
func (p *PostgresKeeper) createRecoveryOptions(recoveryMode dbmgr.RecoveryMode, standbySettings *cluster.StandbySettings, archiveRecoverySettings *cluster.ArchiveRecoverySettings, recoveryTargetSettings *cluster.RecoveryTargetSettings) *dbmgr.RecoveryOptions {
	parameters := common.Parameters{}
...
var _ KeeperIF = (*PostgresKeeper)(nil) // check compatible only

func NewPostgresKeeper(cfg *postgresConfig, end chan error) (KeeperIF, error) {
...
func postgresKeeper(c *cobra.Command, args []string) {
	var (
		err           error
...
func (p *PostgresKeeper) GetPGState(pctx context.Context) (*cluster.DBMSState, error) {
...
func (p *PostgresKeeper) getLastPGState() *cluster.DBMSState {
	p.pgStateMutex.Lock()
...
func (p *PostgresKeeper) isDifferentTimelineBranch(followedDB *cluster.DB, pgState *cluster.DBMSState) bool {
